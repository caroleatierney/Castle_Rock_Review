...
let urlPath = `https://www.googleapis.com/books/v1/volumes?q=inauthor:${encodeURIComponent(author)}&maxResults=40&key=${apiKey}`;
  ...
  $.ajax({
    ...
    $(".button").on("click", (event) => {
      ...
      $(".open").on("click", (event) => {
        ...
        let maturityRating = $(
          `<h3>Maturity Rating: ${results[bookId].maturityRating}</h3>`,
        );
